...
void eval(char *cmdline) 
{
    char buf[MAXLINE];                                  /* 用于解析的命令行 */
    char *argv[MAXARGS];                                /* 存储参数 */
    int bg;                                             /* 是否在后台运行 */
    pid_t pid;                                          /* 进程ID */
    strcpy(buf, cmdline);
    ...
    if (builtin_cmd(argv)) {                            /* 如果是内建命令 */
        return;
    }

    Sigprocmask(SIG_BLOCK, &mask_one, &prev_mask);      /* 阻塞SIGCHLD信号 */
    if ((pid = Fork()) == 0) {                            /* 子进程 */
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
        ...
        if (WIFEXITED(status) || WIFSIGNALED(status)) { /* 终止 */
            if (WIFSIGNALED(status)) {                  /* 因信号而终止则打印，正常终止则直接delete */
